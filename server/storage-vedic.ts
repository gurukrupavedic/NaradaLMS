import {
  users, tracks, chapters, audioFiles, textSegments, audioMappings, studentProgress,
  type User, type UpsertUser, type Track, type InsertTrack, type Chapter, type InsertChapter,
  type AudioFile, type InsertAudioFile, type TextSegment, type InsertTextSegment,
  type AudioMapping, type InsertAudioMapping, type StudentProgress, type InsertStudentProgress
} from "@shared/schema";
import { db } from "./db";
...
export interface IStorage {
  ...
  inviteUser(adminId: string, userData: Partial<UpsertUser>): Promise<User>;
...
export class DatabaseStorage implements IStorage {
  ...
  async inviteUser(adminId: string, userData: Partial<UpsertUser>): Promise<User> {
    const [user] = await db
      .insert(users)
      .values({
        ...userData,
  ...
  async deleteAudioMapping(audioFileId: number, segmentId: number): Promise<void> {
    await db.delete(audioMappings).where(
      and(
  ...
  async getAllStudentProgress(): Promise<(StudentProgress & { student: User; chapter: Chapter & { track: Track } })[]> {
    ...
    // TODO: Implement proper joins with relations
    return allProgress as any;
  }
