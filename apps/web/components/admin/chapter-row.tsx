    ...
    <li className="border-b border-rule-soft last:border-0">
      ...
      {isEditing && (
        <div className="border-t border-rule-soft bg-ink/[0.02] px-4 py-4">
          ...
          {/* Content authoring (script text, segmentation, audio mapping) is enough surface area
              to want its own screen — `chapter-editor.tsx`'s own doc comment has the reasoning. */}
          <div className="mt-5 border-t border-rule-soft pt-4">
            <Link
              href={`/chapter-editor/${chapter.id}`}
              className="label inline-flex items-center gap-1.5 border border-rule px-2.5 py-1.5 text-ink-muted transition-colors hover:border-vermilion hover:text-vermilion"
            >
              Edit content →
            </Link>
          </div>
