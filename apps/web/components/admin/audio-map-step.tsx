...
function useWaveformMapper({ url }: { url: string }) {
  ...
  return { containerRef, regions: regionsRef, time, duration, playing, toggle, seek, pxPerSec, setPxPerSec }
}
...
function useSyncRegions({
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mappings, segments, armedSegmentId])
}
...
function ArmedAudioMapper({ chapterId, asset, segments }: { chapterId: string; asset: ApiAudioAsset; segments: MappableSegment[] }) {
  ...
  const player = useWaveformMapper({ url: asset.url })
  ...
  useSyncRegions({ regions: player.regions, mappings, segments, armedSegmentId: armedSegment?.id ?? null })

  useEffect(() => {
    const plugin = player.regions.current
    if (!plugin) return
  ...
  return (
    <div ref={containerRef} tabIndex={-1} className="border border-rule-soft p-4">
      ...
      <div className="mt-4 border-t border-rule-soft pt-3.5">
        ...
        <div ref={player.containerRef} className="mt-2 w-full overflow-x-auto border border-rule-soft" />
