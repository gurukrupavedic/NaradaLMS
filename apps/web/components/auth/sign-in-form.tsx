...
type Step = 'sign-in' | 'profiles'

export function SignInForm({ initialProfiles }: { initialProfiles?: ApiProfile[] }) {
  const [step, setStep] = useState<Step>(initialProfiles ? 'profiles' : 'sign-in')
  const [profiles, setProfiles] = useState<ApiProfile[]>(initialProfiles ?? [])
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isSelected ? 'border-primary' : 'border-border bg-card hover:border-foreground/25',
                )}
              >
                <div
                  className={cn(
