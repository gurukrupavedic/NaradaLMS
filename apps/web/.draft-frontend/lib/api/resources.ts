 ...
 * they call the real apps/api-next as the signed-in profile (see `lib/api/client.ts`'s docstring
 ...
 * "── Mutations ──" below remain mocked — those endpoints don't exist in apps/api-next yet (out of
...
// in apps/api-next specifically because this page needed it, see PARITY_PLAN.md).
...
// separate by-code lookup endpoint (apps/api-next's GET /batches/:batchId takes a UUID, not a
...
// ── Content authoring ──────────────────────────────────────────────────────
// Real writes against apps/api-next's chapters domain — unlike every mutation above, these have a
