...
router.get(
  ...
  profileRoute(async ({ req, res, db, access }) => {
    ...
    const exams = await findExams(query, visibility, db)
...
router.get(
  ...
  profileRoute(async ({ req, res, db, access }) => {
    ...
    const exam = await findById(examId, db)
...
router.post(
  ...
  profileRoute(async ({ req, res, db, access }) => {
    ...
    const exam = await createExam(data, db)
...
router.post(
  ...
  profileRoute(async ({ req, res, db, access, profile }) => {
    ...
    const exam = await recordExamResult(examId, profile.id, data, db)
    res.status(200).json({ data: exam })
  }),
)
