 ...
 * Used to drive apps/web/.draft-frontend against this real api-next server without a real login
 * flow (that workspace has none yet).
 *
 * Run from apps/api-next so `@narada/env/load` picks up the shared root `.env`:
 *   pnpm --filter @narada/api-next exec tsx scripts/mint-dev-sessions.ts
 *
