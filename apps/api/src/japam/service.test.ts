...
const slmts = { db, school: { slug: 'slmts' } }
const own = { ...slmts, user, access: access(false) }
const admin = { ...slmts, user, access: access(true) }
const rr = { db, school: { slug: 'rr' }, user, access: access(false) }
...
describe('a school without japam', () => {
  it('has no such resource: every operation 404s before touching the database', async () => {
    await expect(summary(rr, 'p1', {})).rejects.toMatchObject({ statusCode: 404 })
    await expect(log(rr, 'p1', { count: 108 })).rejects.toMatchObject({ statusCode: 404 })
    await expect(setDay(rr, 'p1', '2026-03-01', 5)).rejects.toMatchObject({ statusCode: 404 })
    expect(repository.findLoggableProfile).not.toHaveBeenCalled()
  })
})

describe('summary', () => {
  it("returns the totals with the student's own today", async () => {
    ...
    await expect(summary(slmts, 'p1', { from: '2026-01-01' })).resolves.toEqual({
      today: '2026-03-02',
      total: 216,
      lifetime: 1080,
      days: [{ loggedOn: '2026-03-02', count: 216 }],
    ...
    expect(repository.summarize).toHaveBeenCalledWith(db, 'p1', { from: '2026-01-01' })
  })

  it('404s a missing or deactivated profile', async () => {
    vi.mocked(repository.findLoggableProfile).mockResolvedValue(undefined)
    await expect(summary(slmts, 'p1', {})).rejects.toMatchObject({ statusCode: 404 })
  })
})

describe('log', () => {
  it("adds to the student's today, scoped to the caller as owner", async () => {
    vi.mocked(repository.findLoggableProfile).mockResolvedValue({ countryTimeZone: 'Asia/Kolkata' })
    vi.mocked(repository.addToDay).mockResolvedValue({ loggedOn: '2026-03-02', count: 108 })

    await expect(log(own, 'p1', { count: 108 })).resolves.toEqual({
      loggedOn: '2026-03-02',
      count: 108,
    })
    expect(repository.findLoggableProfile).toHaveBeenCalledWith(db, 'p1', 'user-1')
    expect(repository.addToDay).toHaveBeenCalledWith(db, 'p1', '2026-03-02', 108)
  })
  ...
  it("uses the profile's time zone, so the same instant is a different day elsewhere", async () => {
    ...
    await log(own, 'p1', { count: 1 })

    expect(repository.addToDay).toHaveBeenCalledWith(db, 'p1', '2026-03-01', 1)
  })
  ...
  it('accepts a past day, for a sitting that was forgotten', async () => {
    ...
    await log(own, 'p1', { count: 54, loggedOn: '2026-02-20' })

    expect(repository.addToDay).toHaveBeenCalledWith(db, 'p1', '2026-02-20', 54)
  })
  ...
  it("refuses a future day, judged by the student's own today", async () => {
    ...
    await expect(log(own, 'p1', { count: 1, loggedOn: '2026-03-02' })).rejects.toMatchObject({
      statusCode: 422,
    })
  ...
  it("404s someone else's profile for a non-admin, and writes nothing", async () => {
    ...
    await expect(log(own, 'p1', { count: 1 })).rejects.toMatchObject({ statusCode: 404 })
    expect(repository.addToDay).not.toHaveBeenCalled()
  })
  ...
  it("turns the database's daily-cap violation into a 422", async () => {
    ...
    vi.mocked(repository.addToDay).mockRejectedValue(
      Object.assign(new Error('violates check'), {
        code: '23514',
        constraint: 'japamLog_count_valid',
      }),
    )

    await expect(log(own, 'p1', { count: 1 })).rejects.toMatchObject({
      statusCode: 422,
...
describe('setDay', () => {
  it("sets a past day's total outright", async () => {
    ...
    await expect(setDay(own, 'p1', '2026-02-20', 30)).resolves.toEqual({
      loggedOn: '2026-02-20',
      count: 30,
    })
    expect(repository.findLoggableProfile).toHaveBeenCalledWith(db, 'p1', 'user-1')
    expect(repository.setDay).toHaveBeenCalledWith(db, 'p1', '2026-02-20', 30)
  })
  ...
  it('refuses a future day', async () => {
    ...
    await expect(setDay(own, 'p1', '2026-03-02', 5)).rejects.toMatchObject({ statusCode: 422 })
  ...
  it("404s someone else's profile for a non-admin", async () => {
    ...
    await expect(setDay(own, 'p1', '2026-02-20', 5)).rejects.toMatchObject({ statusCode: 404 })
  })
})
