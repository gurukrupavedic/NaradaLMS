...
| `POST /exams/:examId/results`                  | Present (2026-09-15)          | **Deliberate divergence from §11.6**: `requireCanRecordEvaluation` no longer delegates to `requireCanUpdateExam` — it's school-admin only (owner/admin/super-admin), independent of any batch role. Product decision (2026-09-15): a batch instructor/TA grades absent/L1-L3 via their own evaluation, but L4 (the only outcome an exam result can record — see `evaluations/schema.ts`'s `teacherGradableLevelSchema` / `exams/schema.ts`'s `RecordExamResultSchema`) is admin-only to record. Single-result transaction hardening retained unchanged. |
...
bypasses there — a plain owner/admin with no batch role is denied). (`requireCanRecordEvaluation`
later diverged from this row entirely — see §2's `POST /exams/:examId/results` row, 2026-09-15: it
is now school-admin only by design, not batch-role gated at all.) Batches and evaluations
