...
export function TrackTabs({ tracks, selectedTrackId, currentTrackId, onSelectTrack, isLoading = false, children }: TrackTabsProps) {
    if (isLoading) {
        return (
        ...
        <Tabs value={selectedTrackId} onValueChange={onSelectTrack} className="w-full">
            <TabsList className="w-full justify-start overflow-x-auto bg-muted p-1 scrollbar-hide">
                {tracks.map((track) => {
                    ...
                    return (
                        <TabsTrigger key={track.id} value={String(track.id)} className="flex items-center gap-2 whitespace-nowrap data-[state=active]:bg-background">
                            <span className="text-sm font-medium">Track {track.order}</span>
                            {isCurrentTrack && <Badge variant="secondary" className="text-[10px] px-1.5 py-0">Current</Badge>}
                        </TabsTrigger>
                    );
                })}
            </TabsList>
