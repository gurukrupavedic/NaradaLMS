...
export function UnifiedBatchMatrix({
    ...
    const [updatingCell, setUpdatingCell] = useState<{ studentId: string; chapterId: string } | null>(null);

    const progressMap = useMemo(() => {
        const map = new Map<string, StudentProgress>();
        progress.forEach((p) => map.set(`${p.studentId}-${p.chapterId}`, p));
        return map;
    }, [progress]);
    ...
    const getMatrixCell = (studentId: string, chapterId: string): MatrixCell => {
        ...
        if (!prog) {
            return { studentId, chapterId, proficiencyLevel: 9, status: "not_started", isEmpty: true };
        }
        return { studentId, chapterId, proficiencyLevel: prog.proficiencyLevel, status: prog.status, isEmpty: false };
    };
    ...
    const handleDropStudent = async (enrollmentId: number, studentName: string) => {
        try {
            ...
            toast({ title: "Failed to remove student", description: err instanceof Error ? err.message : "An error occurred", variant: "destructive" });
        }
    };
    ...
    const handleUpdateProficiency = async (level: ProficiencyLevel, notes?: string) => {
        ...
        try {
            ...
            toast({ title: "Failed to update proficiency", description: err instanceof Error ? err.message : "An error occurred", variant: "destructive" });
        } finally {
    ...
    const getInitialBgColor = (studentId: string): string => {
        const colors = ["bg-blue-500", "bg-green-500", "bg-purple-500", "bg-orange-500", "bg-red-500", "bg-indigo-500", "bg-pink-500", "bg-cyan-500"];
            ...
            cell: (info) => {
                ...
                return (
                    <div className="pl-4 pr-2 py-2 flex items-center gap-2 min-w-0">
                        <div className={`${getInitialBgColor(student.id)} text-white font-bold rounded-full w-8 h-8 flex items-center justify-center flex-shrink-0 text-xs`} title={`${student.firstName} ${student.lastName}`}>
                            {getInitials(student.firstName, student.lastName)}
                        </div>
                        <div className="min-w-0">
                            <div className="truncate font-medium text-sm">{student.firstName} {student.lastName}</div>
                    ...
                    <div className="flex items-center justify-center">
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="ghost" size="icon" className="data-[state=open]:bg-muted h-8 w-8" title="Student actions menu">
                                    <MoreVertical className="h-4 w-4" />
                                    <span className="sr-only">Student actions</span>
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end" className="bg-popover border border-border shadow-lg min-w-[180px]">
                                <DropdownMenuItem onClick={() => handleDropStudent(student.enrollmentId, `${student.firstName} ${student.lastName}`)} className="text-destructive focus-visible:text-destructive">
                                    Drop Student
                                </DropdownMenuItem>
                            </DropdownMenuContent>
                        </DropdownMenu>
            ...
            columnHelper.accessor((row) => row.id, {
                ...
                header: () => (
                    <div className="px-2 py-2 flex flex-col items-center justify-center gap-0.5">
                        <div className="text-xs font-bold text-gray-900 whitespace-nowrap text-center">{chapter.code}</div>
                        <div className="text-[10px] text-gray-600 line-clamp-2 text-center" title={chapter.title}>{chapter.title}</div>
                    </div>
                ),
                cell: (info) => {
                    ...
                    return (
                        <div className="px-2 py-2 flex items-center justify-center">
                            <button
                                ...
                                className={`h-14 w-20 flex items-center justify-center rounded-lg border-2 transition-colors relative ${colors.bgColor} ${colors.darkBgColor} ${colors.textColor} ${colors.darkTextColor} ${colors.borderColor} ${colors.darkBorderColor} ${canEditProficiency && !isCellUpdating ? "cursor-pointer hover:shadow-md" : "cursor-not-allowed"} ${isCellUpdating ? "opacity-60" : ""} font-semibold text-sm`}
                                title={canEditProficiency ? `${info.row.original.firstName} - ${chapter.code}` : "Only instructors can update proficiency"}
                            >
                                {isCellUpdating ? <Loader className="h-4 w-4 animate-spin" /> : getProficiencyShortLabel(cell.proficiencyLevel)}
                            </button>
                        </div>
                    );
                },
        ...
        <div className="space-y-4">
                            ...
                            <tr key={headerGroup.id} className="border-b-2 border-border bg-muted/50">
                                {headerGroup.headers.map((header) => {
                                    ...
                                    return (
                                        <th
                                            key={header.id}
                                            className={`${isStudentCol ? "text-center" : "text-center"} text-xs font-semibold text-muted-foreground uppercase tracking-tight ${isStudentCol ? "pl-4 pr-2 py-2 align-middle" : "p-0"} sticky top-0 ${isSticky ? "z-30" : "z-10"} bg-card`}
                                            style={{ width: `${header.getSize()}px`, maxWidth: `${header.getSize()}px`, ...(isSticky && { left: stickyLeftStyle }) } as React.CSSProperties}
                                        >
                            ...
                            <tr>
                                <td colSpan={columns.length} className="p-12 text-center text-muted-foreground border-b border-border">
                                    <div className="flex flex-col items-center justify-center gap-2">
                                        ...
                                        <div className="text-xs">Add students to get started</div>
                                    </div>
                                </td>
                            </tr>
                        ) : (
                                    ...
                                    {row.getVisibleCells().map((cell) => {
                                        ...
                                        return (
                                            <td
                                                ...
                                                style={{ width: `${cell.column.getSize()}px`, maxWidth: `${cell.column.getSize()}px`, ...(isSticky && { left: stickyCellLeftStyle }) } as React.CSSProperties}
                                            >
            ...
            {selectedCell && (
                <MatrixEvaluationModal
                    ...
                    currentProficiency={progressMap.get(`${selectedCell.studentId}-${selectedCell.chapterId}`)?.proficiencyLevel}
                    onClose={() => { setModalOpen(false); setSelectedCell(null); }}
