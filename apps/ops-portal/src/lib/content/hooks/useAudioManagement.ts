...
export function useAudioManagement(chapterId: string) {
    ...
    const uploadMutation = useMutation({
        mutationFn: async (file: File) => {
            ...
            // Upload using apiRequest (handles CSRF and Authorization automatically)
            // apiRequest will detect FormData and skip Content-Type header
            const response = await apiRequest(`/content/chapters/${chapterId}/audio`, {
                method: 'POST',
                body: formData,
            });
