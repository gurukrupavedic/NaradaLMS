...
import { ScrollArea } from '@narada/ui';
import {
  ResizableHandle,
  ResizablePanel,
  ResizablePanelGroup,
...
import { Textarea } from '@narada/ui';
import {
  Select,
  SelectContent,
  SelectItem,
...
import { handleJsonResponse } from '@/lib/content/handleJsonResponse';
...
export default function TracksAndChapters() {
  ...
  const tracksQuery = useQuery<TrackRow[]>({
    ...
    queryFn: async () => {
      const res = await apiRequest('/api/content/tracks', { method: 'GET' });
  ...
  const chaptersQuery = useQuery<ChapterRow[]>({
    ...
    queryFn: async () => {
      const res = await apiRequest(`/api/content/tracks/${selectedTrackId}/chapters`, { method: 'GET' });
  ...
  const createTrackMutation = useMutation({
    mutationFn: async (payload: { title: string; description: string }) => {
      const res = await apiRequest('/api/content/tracks', { method: 'POST', body: JSON.stringify(payload) });
  ...
  const updateTrackMutation = useMutation({
    mutationFn: async ({ trackId, title, description }: { trackId: number; title: string; description: string }) => {
      const res = await apiRequest(`/api/content/tracks/${trackId}`, { method: 'PUT', body: JSON.stringify({ title, description }) });
  ...
  const deleteTrackMutation = useMutation({
    mutationFn: async (trackId: number) => {
      const res = await apiRequest(`/api/content/tracks/${trackId}`, { method: 'DELETE' });
  ...
  const moveTrackMutation = useMutation({
    mutationFn: async ({ trackId, direction, steps }: { trackId: number; direction: 'up' | 'down'; steps: number }) => {
      for (let i = 0; i < steps; i++) {
        const res = await apiRequest(`/api/content/tracks/${trackId}/move`, { method: 'POST', body: JSON.stringify({ direction }) });
  ...
  const createChapterMutation = useMutation({
    mutationFn: async ({ trackId, title, description }: { trackId: number; title: string, description: string }) => {
      const res = await apiRequest(`/api/content/tracks/${trackId}/chapters`, { method: 'POST', body: JSON.stringify({ title, description, content: {} }) });
  ...
  const updateChapterMutation = useMutation({
    mutationFn: async ({ chapterId, title, description }: { chapterId: number; title: string, description: string }) => {
      const res = await apiRequest(`/api/content/chapters/${chapterId}`, { method: 'PUT', body: JSON.stringify({ title, description }) });
  ...
  const deleteChapterMutation = useMutation({
    mutationFn: async ({ chapterId }: { chapterId: number; trackId?: number }) => {
      const res = await apiRequest(`/api/content/chapters/${chapterId}`, { method: 'DELETE' });
  ...
  const moveChapterMutation = useMutation({
    mutationFn: async ({ chapterId, toTrackId }: { chapterId: number; toTrackId: number }) => {
      const res = await apiRequest(`/api/content/chapters/${chapterId}/move`, { method: 'POST', body: JSON.stringify({ toTrackId }) });
  ...
  const reorderChapterMutation = useMutation({
    mutationFn: async ({ chapterId, direction, steps }: { chapterId: number; direction: 'up' | 'down'; steps: number }) => {
      for (let i = 0; i < steps; i++) {
        const res = await apiRequest(`/api/content/chapters/${chapterId}/move`, { method: 'POST', body: JSON.stringify({ direction }) });
