...
export function TimestampControl({
    ...
    const formatTimeVal = (time: number) => formatDuration(time, { showDecimal: true });

    const handleStartEdit = () => {
        if (readOnly) return;
        onEditStart();
        if (isMapped) {
            setEditStartStr(formatTimeVal(startTime));
            setEditEndStr(formatTimeVal(endTime));
        } else {
            const suggestedStart = isFirstSegment ? 0 : previousSegmentEndTime;
            setEditStartStr(formatTimeVal(suggestedStart!));
            setEditEndStr('');
        }
    };
    ...
    const handleSave = () => {
        ...
        if (isMapped && onUpdate) {
            ...
            onCreate({ segmentId: segmentId, startTime: newStart, endTime: newEnd } as AudioMapping);
        }
        onEditCancel();
    };
