...
function transform(
  server: BatchProgressResponseServer
): BatchProgressResponseUI {
  ...
  const chapters = Array.from(chapterIndex.values()).sort(
...
export function useBatchProgress(batchId: number | string | undefined) {
  const queryKey = batchId
    ...
    : "/batches/undefined/progress";
  return useQuery<BatchProgressResponseUI>({
    queryKey: [queryKey],
    queryFn: async () => {
      const data =
        await apiRequest<BatchProgressResponseServer>(queryKey);
