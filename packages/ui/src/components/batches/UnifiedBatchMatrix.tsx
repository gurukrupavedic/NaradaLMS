...
export function UnifiedBatchMatrix({
  ...
  const handleUpdateProficiency = async (
    ...
    try {
      ...
      await onUpdateProficiency(
        selectedCell.studentId,
        selectedCell.chapterId,
        level,
        notes
      );
      ...
      cell: (info) => {
        ...
        return (
          <div className="pl-4 pr-2 py-2 flex items-center gap-2 min-w-0">
            <div
              className={`${getInitialBgColor(student.id)} text-white font-bold rounded-full w-8 h-8 flex items-center justify-center flex-shrink-0 text-xs`}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="data-[state=open]:bg-muted h-8 w-8"
      ...
      columnHelper.accessor((row) => row.id, {
        ...
        header: () => (
          <div className="px-2 py-2 flex flex-col items-center justify-center gap-0.5">
            ...
            <div
              className="text-[10px] text-gray-600 line-clamp-2 text-center"
              title={chapter.title}
            >
              {chapter.title}
            </div>
        ...
        cell: (info) => {
          ...
          const isCellUpdating =
            updatingCell?.studentId === studentId &&
            updatingCell?.chapterId === chapter.id;

          return (
            <div className="px-2 py-2 flex items-center justify-center">
              <button
                onClick={() =>
                  canEditProficiency && handleCellClick(studentId, chapter.id)
                }
                disabled={
                  isUpdating || !canEditProficiency || isCellUpdating
                }
                className={`
                ...
                title={
                  canEditProficiency
                    ? `${info.row.original.firstName} - ${chapter.code}`
    ...
    <div className="space-y-4">
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              ...
              >
                {headerGroup.headers.map((header) => {
                  const isSticky =
                    header.id === "student" || header.id === "actions";
                  const isStudentCol = header.id === "student";
                  const stickyLeftStyle = isSticky
            ...
            {table.getRowModel().rows.length === 0 ? (
              <tr>
                <td
                  ...
                  className="p-12 text-center text-muted-foreground border-b border-border"
                >
                  <div className="flex flex-col items-center justify-center gap-2">
                    ...
                    <div className="text-xs">
                      Add students below to get started
                    </div>
                  </div>
                </td>
              </tr>
              ...
              table.getRowModel().rows.map((row) => (
                ...
                >
                  {row.getVisibleCells().map((cell) => {
                    ...
                    const stickyCellLeftStyle = isSticky
      ...
      {selectedCell && (
        <MatrixEvaluationModal
          ...
          currentProficiency={
            progressMap.get(
              `${selectedCell.studentId}-${selectedCell.chapterId}`
            )?.proficiencyLevel
          }
          onClose={() => {
            setModalOpen(false);
