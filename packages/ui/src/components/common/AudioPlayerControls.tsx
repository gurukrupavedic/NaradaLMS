...
export function AudioPlayerControls({
  ...
  if (variant === "minimal") {
    return (
      <div
        className={cn(
          "w-full flex items-center gap-3 bg-background border rounded-lg px-3 py-2",
          className
        )}
      >
        {/* Play/Pause Button */}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-7 px-1.5 text-[10px] font-medium text-muted-foreground hover:text-foreground gap-0.5 min-w-[2.5rem]"
                  disabled={disabled}
                >
                  {playbackRate}x
                </Button>
                ...
                {playbackRates.map((rate) => (
                  <DropdownMenuItem
                    ...
                    className={cn(
                      ...
                      playbackRate === rate && "font-bold bg-accent"
                    )}
                  >
                    {rate}x
                  </DropdownMenuItem>
  ...
  return (
    <div
      className={cn(
        "w-full bg-background rounded-xl border p-4 space-y-4",
        className
      )}
    >
