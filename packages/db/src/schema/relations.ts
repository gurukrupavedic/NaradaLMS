...
export const examRelations = relations(exam, ({ one }) => ({
  ...
  // The admin exams screen's own name column — eager-loaded directly here rather than the caller
  // cross-referencing a separate roster fetch (which is itself paginated and can be incomplete for
  // a large school).
