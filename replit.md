...
- June 25, 2025. SWITCH COMPONENT SURGICAL REFINEMENT: Applied surgical precision fixes based on visual analysis of all three sizes. Corrected switch proportions (default h-6→h-6 w-12→w-11, lg h-8→h-7 w-14→w-12), fixed thumb translation distances for proper positioning, refined status text positioning to prevent overlap with thumb movement, reduced glow intensity for better visual hierarchy (20px→15px, 25%→18% on hover), decreased inner thumb glow opacity (30%→20%) for subtlety, and improved font sizing consistency across variants. Status text now uses proper positioning with scale transforms and improved contrast.
