...
- **Ops portal (port 3001):** Admin only. Admin Center includes Tracks & Chapters (content) at `/content`, User Management, Batch Admin, Audit Logs, Settings.
...
- **docs/essentials/product-guide.md:** Replace "student, instructor, content_manager, admin" with "student, instructor, admin"; update §3.1 roles table to three roles (Admin includes content); §3.3 content workflow → "Admins" and ops portal paths; role-guard table: Tracks & Chapters `['admin']`, Instructor in student portal; schema/users comment and roadmap item.
