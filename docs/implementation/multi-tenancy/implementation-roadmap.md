...
- **Next up:** checklist **6.1** pilot validation is now confirmed locally, so default to **6.2** RR isolation smoke next, then **6.3** second-org join verification, unless you intentionally want to keep pushing Layer 4 with broader auth-client or OAuth tenant propagation beyond the current register flow.
