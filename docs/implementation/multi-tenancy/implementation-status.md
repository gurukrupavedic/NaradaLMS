...
| **2.4** Super-admin governance | **`requireSuperAdmin`** on [`/api/auth/admin/users`](../../../server/routes/identity.routes.ts) list + detail + membership lifecycle + super-admin grant/revoke. **`GET /api/auth/admin/users`** returns users with nested **`memberships[]`**; filters **`membershipStatus`** / legacy `status` tab mapping / `role`→membership role / `orgSlug`. **`POST/PATCH …/memberships/:id`** approve, reject, disable, enable, roles. **`GET /api/admin/directory/users`** ([`admin.routes.ts`](../../../server/routes/admin.routes.ts)): org admins list directory in **`req.orgId`** (`membershipRole` + `search`). Admin portal [`UserList`](../../../apps/admin-portal/src/components/admin/UserList.tsx) super-admin gate + membership actions. | [`server/shared/middleware/auth.ts`](../../../server/shared/middleware/auth.ts), [`server/routes/identity.routes.ts`](../../../server/routes/identity.routes.ts), [`server/routes/admin.routes.ts`](../../../server/routes/admin.routes.ts), [`apps/admin-portal/src/lib/hooks/useAdminUsers.ts`](../../../apps/admin-portal/src/lib/hooks/useAdminUsers.ts) |
...
| Roadmap **2.5** event/audit alignment | **2.5** | Membership events published; dedicated audit rows / full bus alignment follow-up. |
...
- **Smoke (optional, server running):** `npx tsx scripts/test/api-smoke-test.ts` — includes **`GET /api/auth/admin/users`** (super-admin, `memberships[]` on users) and **`GET /api/admin/directory/users`** when seeded admin login succeeds; plus **`POST /api/auth/switch-org`** checks as in slice 2.3.
