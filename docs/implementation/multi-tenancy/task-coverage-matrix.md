...
| 3   | MT-1.1   | Schema        | Individual    | `organizations` + `user_organizations` + `users.is_super_admin`; drop `users.roles`/`users.status` | Drizzle + SQL migration                                         | schema matches [schema-design.md](./schema-design.md) |
...
| 16  | MT-3.A.1 | Data          | Individual    | `org_id` Pass A tables + backfill + uniques                                                        | migrations                                                      | NOT NULL enforced                                     |
| 17  | MT-3.A.2 | API           | Individual    | Handlers filter Pass A by org                                                                      | route modules                                                   | isolation spot-check                                  |
| 18  | MT-3.B.1 | Data          | Individual    | `org_id` Pass B + `audit_logs` nullable org                                                        | migrations                                                      | indexes                                               |
| 19  | MT-3.B.2 | API           | Individual    | Handlers filter Pass B by org                                                                      | route modules                                                   | no cross-org joins                                    |
| 20  | MT-3.B.3 | Data          | Individual    | Enrollment/progress uniqueness org-scoped if required                                              | migration + queries                                             | one-active rule per org semantics                     |
| 21  | MT-4.1   | Student UI    | Individual    | Tenant config module + `TENANT` env                                                                | `config/tenants`                                                | two ports show two brands                             |
| 22  | MT-4.2   | Student UI    | Parallel-safe | Replace hardcoded SLMTS assets                                                                     | components                                                      | visual parity SLMTS                                   |
| 23  | MT-4.3   | Client        | Parallel-safe | Tenant header on auth API calls                                                                    | student `api` layer                                             | correct org on register                               |
| 24  | MT-5.1   | Admin UI      | Individual    | Super-admin gate on User Management                                                                | admin routes/pages                                              | non-super-admin blocked                               |
| 25  | MT-5.2   | Admin UI      | Parallel-safe | Multi-org user list + org filter                                                                   | grid + query params                                             | matches API                                           |
| 26  | MT-6.1   | Pilot         | Individual    | Pilot checklist execution                                                                          | notes in [verification-strategy.md](./verification-strategy.md) | sign-off                                              |
