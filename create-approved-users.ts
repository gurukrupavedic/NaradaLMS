...
const roles = ['student', 'instructor', 'content_manager', 'admin'];

function getRandomRole(): string[] {
  // Randomly pick 1-2 roles for variety
    ...
    for (let i = 11; i <= 30; i++) {
      ...
      try {
        const result = await client.query(
          `INSERT INTO users (email, first_name, last_name, password_hash, provider, status, roles, approved_at, approved_by, created_at, updated_at)
           VALUES ($1, $2, $3, $4, 'local', 'active', $5::text[], NOW(), $6, NOW(), NOW())
           RETURNING id, email, first_name, last_name, roles`,
          [email, firstName, lastName, passwordHash, assignedRoles, kashyapId]
        );
        
        const user = result.rows[0];
        console.log(`✅ Created & Approved: ${user.first_name} ${user.last_name} (${user.email}) - Roles: ${user.roles.join(', ')}`);
        created++;
      } catch (error: any) {
